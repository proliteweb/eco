...
$DB_CONNECT_SETTINGS = ["host" => "127.0.0.1", "user" => "prolite", "pass" => "oleg1993", "dbName" => "proliteweb"];
 ...
 * @param $table - название таблицы
 * @param $array - массив с данными для вставки, должен совпадать с структурой указонной таблицы
 * @param $where - условие, куда добавлять
 * @param bool|false $close
 */
function db_insert_where($table, $array, $where, $close = false){

    if(!$array or ! $table or !$where){ exit("Ошибка: не верные параметры. На строке:". __LINE__ ); }
    global $DB;

    $sql = "INSERT INTO ".$table." (".implode(",", array_keys($array)).") VALUES ('".implode("','", array_values($array))."') WHERE ".$where;
    $res = db_query($sql);
